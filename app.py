...
# importing model
# Skip model loading for now to get the app running
...
def chart():
    ...
    return render_template('cindex.html',district=district, map_html=map_html,line_json=line_json,pie_json=pie_json,chart_json=chart_json, crops=crops, max_crop=max_col, top_5=top_5, top_districts=top_districts)
...
@app.route('/buy')
def buy():
    crops = list(mongo.db.trades.find())
    for crop in crops:
        image_path = os.path.join(app.config['UPLOAD_FOLDER'], crop['crop_image'])
        if not os.path.exists(image_path):
            # If the image file doesn't exist, try to use an SVG placeholder
            base_name = os.path.splitext(crop['crop_image'])[0]
            svg_path = os.path.join(app.config['UPLOAD_FOLDER'], base_name + '.svg')
            if os.path.exists(svg_path):
                crop['crop_image'] = base_name + '.svg'
            else:
                # Fallback to a default image if no SVG placeholder exists
                crop['crop_image'] = 'Logo.png' # Or any other default image you have
...
def add_to_list():
    ...
    product = db.trades.find_one({'_id': ObjectId(product_id)}) or \
              {'product_name': product_name, 'price_per_kg': float(request.form.get('price', 0)), 'crop_image': request.form.get('crop_image', 'Logo.png')}

    if product:
        product['price_per_kg'] = float(product['price_per_kg'])  # Convert to float
...
def shopping_list():
    ...
    total_price = sum([product['price_per_kg'] * product.get('quantity', 1) for product in shopping_list])
